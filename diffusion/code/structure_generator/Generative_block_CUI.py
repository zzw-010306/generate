...
# 导入外部模块
from generator_diffusion_functions import Diffusion_predict
...
class hybrid_generator_multi:
    def __init__(self, scaffold_smiles, expand_index, block_list, start_fragment_smiles_list=["C"]):
        """初始化混合生成器"""
        model_path = "path/to/your/model"  # 请替换为实际模型路径
        label_path = "fragment_label.csv"
        self.diffusion = Diffusion_predict(model_path, label_path)
...
if __name__ == "__main__":
    ...
    Generator.one_epoch_generate(generate_num=1000, block_priority=priority_structure, select_fragment_num=1000)
    print('imok')
